...
public let GTK_TYPE_HSCALE: GType = gtk_hscale_get_type()

@inline(__always) public func GTK_HSCALE(_ ptr: UnsafeMutableRawPointer!) -> UnsafeMutablePointer<GtkHScale>! {
	return G_TYPE_CHECK_INSTANCE_CAST(ptr, GTK_TYPE_HSCALE)
}
...
open class CGTKHScale : CGTKScale {
	...
	public convenience init(adjustment: UnsafeMutablePointer<GtkAdjustment>?) {
		self.init(withGObject: gtk_hscale_new(adjustment))!
	}
	...
	public convenience init(withRange min: Double, max: Double, step: Double) {
		self.init(withGObject: gtk_hscale_new_with_range(min, max, step))!
	}

	open var HSCALE: UnsafeMutablePointer<GtkHScale>! {
		get {
